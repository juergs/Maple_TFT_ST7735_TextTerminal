...
//--- using MAPLE-Mini-SPI-Interface#1,  
...
// yArea must be a integral multiple of TEXT_HEIGHT = 16 pixels
//uint16_t yArea      = 320-TOP_FIXED_AREA-BOT_FIXED_AREA;

// --- we are using our display in landscape mode (setrotation=0, Y=128 X=160) 
uint16_t yArea   = MAX_Y - TOP_FIXED_AREA - BOT_FIXED_AREA;

//--- initial y coordinate of the top of the bottom text line  
uint16_t yDraw   = MAX_Y - BOT_FIXED_AREA - TEXT_HEIGHT;
...
//---  test variables used during debugging
...
void setup() 
{
  ...
  //--- setup TFT 
  tft.begin();
  tft.setRotation(0);    //--- special modes in this library: 0..3 and 7, see setRotion in lib
  ...
  tft.drawCentreString("Serial Terminal-115K2",60,0,2);
...
int scroll_line() 
{
  ...
  if (yStart >= 128 - BOT_FIXED_AREA) 
    yStart = TOP_FIXED_AREA + (yStart - 128 + BOT_FIXED_AREA);
